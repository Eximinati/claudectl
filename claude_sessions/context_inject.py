...
def run(project_path, proj_folder, project_name):
    """Pick a session (any account, this project) and launch a NEW session
    under the CURRENT account (proj_folder's own) seeded with its transcript.
    Returns True if a session was launched, False if cancelled."""
    ...
    env = os.environ.copy()
    env['CLAUDE_CONFIG_DIR'] = _account_dir_of(proj_folder)
