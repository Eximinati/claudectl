...
- **Change diffs** — after regenerating CLAUDE.md (`c`/`a`), an AI system prompt (`s`), or global MCP docs, claudectl shows a **git-style colored diff** (old → new) of exactly what changed. The previous version is snapshotted under `.claudectl/snapshots/`, so the workspace screen (`w`) lists recent changes with `+/−` counts and re-opens the last diff on `c` (CLAUDE.md) / `s` (system prompt).
